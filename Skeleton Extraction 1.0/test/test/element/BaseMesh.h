...
*
*  这是网格的基本类别。 它包括一个顶点列表，
   边列表，面列表。 所有的几何对象都是通过指针连接的，
  顶点，边，面由半边连接。 Mesh类具有文件IO功能，
支持.obj，.m和.off文件格式。 它为每个几何本原提供了Euler运算符
   可以自由访问其邻居。
*  
...
public:
	...
	*/
	...
	void write_result(const char * output);
	/*!
	Read an .m file.
...
{
	cout << "开始读取文件" << endl;
	filename = "E:\\Skeleton Extraction 1.0\\test\\data\\out.obj";
	...
	std::vector<CPoint2> uvs;//纹理坐标
	std::vector<CPoint> normals;//法向量
	//逐行读取
	while ( f.getline( cmd, 1024) )
    {
		...
		if ( token == "f" )
		{
			...
			for( int i = 0 ; i < 3; i ++ )
			{
				...
				v[i] = m_map_vert[ ids[0] ];//查找到对应点
				if( with_uv )
					v[i]->uv() = uvs[ ids[1]-1 ];
				if( with_normal )
					v[i]->normal() = normals[ ids[2]-1 ];
			}
			createFace( v, fid++ );//创造面（同时生成三边）
		}
	}
	f.close();
	labelBoundary();
...
template<typename CVertex, typename CEdge, typename CFace, typename CHalfEdge>
void CBaseMesh<CVertex, CEdge, CFace, CHalfEdge>::write_result(const char * output)
{
	std::fstream _os(output, std::fstream::out);
	if (_os.fail())
	{
